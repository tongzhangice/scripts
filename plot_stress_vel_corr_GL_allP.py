...
data_c = Dataset(options.input_file,'r')

coord = np.loadtxt(options.coord_file)
xcoord_P = coord[0:perturbN,0]
ycoord_P = coord[0:perturbN,1]

exx_c = data_c.variables['exx'][0,:]
eyy_c = data_c.variables['eyy'][0,:]
exy_c = data_c.variables['exy'][0,:]
eyx_c = data_c.variables['eyx'][0,:]

#thickness_c = data_c.variables['thickness'][0,:]

ux_c = data_c.variables['uReconstructX'][0,:,:]
uy_c = data_c.variables['uReconstructY'][0,:,:]

mean_ux_c = np.mean(ux_c,axis=1)
mean_uy_c = np.mean(uy_c,axis=1)

#nx_flow = mean_ux/np.sqrt(mean_ux**2+mean_uy**2+eps)
#ny_flow = mean_uy/np.sqrt(mean_ux**2+mean_uy**2+eps)


strainRate_e_c = np.sqrt(0.5*(exx_c**2+eyy_c**2) + 0.25*(exy_c+eyx_c)**2)
visc_c = 0.5*A**(-1.0/n)*(strainRate_e_c+eps)**((1.0-n)/n)

sigma_xx_d_c = 2*visc_c*exx_c
sigma_yy_d_c = 2*visc_c*eyy_c
sigma_xy_c = 2*visc_c*(exy_c+eyx_c)/2.0
sigma_yx_c = 2*visc_c*(eyx_c+exy_c)/2.0

sigma_xx_c = 2*sigma_xx_d_c+sigma_yy_d_c
sigma_yy_c = 2*sigma_yy_d_c+sigma_xx_d_c

vel_c = np.sqrt(mean_uy_c**2+mean_uy_c**2)
vel_c_GL = vel_c[indexGL]
...
for j in range(738):
    ...
    if (thkGrad[j]<7e-3) and (xcoord_P[j]<480e3):
        data = Dataset()
        ...
        vel = np.sqrt(mean_uy**2+mean_uy**2)
        vel_GL = vel[indexGL]

        velChange = (vel_GL-vel_c_GL)/vel_c_GL*100.0
    ...
    for i in range(180):
        ...
        theta_principle_c = 0.5*np.arctan(2*sigma_xy_c/(sigma_xx_c-sigma_yy_c))

        nx = np.cos(theta_principle+theta)
        ny = np.sin(theta_principle+theta)

        nx_c = np.cos(theta_principle_c+theta)
        ny_c = np.sin(theta_principle_c+theta)

        sigma_nn = sigma_xx*nx**2+(sigma_xy+sigma_yx)*nx*ny+sigma_yy*ny**2
        sigma_nn_c = sigma_xx_c*nx_c**2+(sigma_xy_c+sigma_yx_c)*nx_c*ny_c+sigma_yy_c*ny_c**2

        sigma_nn_GL = sigma_nn[indexGL]
        sigma_nn_c_GL = sigma_nn_c[indexGL]

        sigmaChange = (sigma_nn_GL-sigma_nn_c_GL)/sigma_nn_c_GL * 100.0
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(sigmaChange,velChange)
        r2_all[i] = r_value*r_value


    r2P[j,0] = np.max(r2_all)
    r2P[j,1] = np.argmax(r2_all)



nonzeroid = np.nonzero(r2P[:,0])[0]
r2P_index_like = r2P[nonzeroid,:]
